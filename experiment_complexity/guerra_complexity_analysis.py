...
# Merged version of network_polarization.py and helpers.py
...
def main_function_polarization (groups, edges, group_ids):

    print('Filter of relevant clusters')
    edges, groups = filter_graph(edges, groups, group_ids)
    ...
    for _id, (a, b) in enumerate(pairs):
        ...
        print('Finding internal nodes')
        internal_nodes = get_internal(p_edges)
        
        print('Finding nodes on boundary')
        boundary_nodes = get_boundaries(p_edges, groups, internal_nodes)
    
        print('Creating set of edges with internal nodes')
        internal_edges = get_internal_edges(p_edges, internal_nodes, boundary_nodes)
        
        print('Creating set of edges with nodes on boundary')
        boundary_edges = get_boundary_edges(p_edges, boundary_nodes)
        
        print('Calculation of nodes polarization')
        nodes_p = nodes_polarization(boundary_nodes, internal_edges, boundary_edges)
    
        print('Calculation of polarization for clusters ' + str(a) + ' e '+ str(b))
        p = polarization(nodes_p)
